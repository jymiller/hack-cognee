...
- **Public artifacts:** the visual deck and browser edition of Scope Goblin are static, hosted pages.
- **Working demo:** a saved, curated method informs Scope Goblin’s deterministic pitch-to-plan rules. The game itself makes no live Cognee or model request.
...
3. Scope Goblin produces that action and check for a pitch, with source metadata.
